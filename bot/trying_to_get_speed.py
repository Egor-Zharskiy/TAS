...
import os
from functions_to_bot import get_all_pupils, delete, get_text_from_the_teacher_in_func, get_files_from_teacher, enter_mark
...
def xppx(message):
    ...
    print(name_of_the_pupil, 'axaxaxxaxaxaxaxax')
...
def ans(message):
    choice = ['файл', 'текст']
    # kb_fruits = keyboa_maker(items=choice, copy_text_to_callback=True, auto_alignment=True)
    bot.send_message(message.chat.id, 'ответьте на тест файлом, в котором содержатся ваши ответы')
...
def answer(message):
    ...
    for i in tables:
        ...
        q = c.execute(select).fetchall()
        for j in q:
#вот тут пиздец, нихуя не работает нахуй сука блять, пизда что делать нахуй.
            if j[0] == id:
            ...
            if len(filenames) > 1:
                ...
                for j in filenames:
                    ...
                    if j.find('test') != -1:
                        ...
                        # print(path_to_file)
                        print(path_to_file, 'path_to_filepath_to_filepath_to_filepath_to_filepath_to_filepath_to_filepath_to_filepath_to_file')
                        print("AXAXAXAXAXAXAX")
                        print("c:/users/hp/PycharmProjects/TAS/files/" + english_name + '/' + file_name2)
                        ...
                        if type(s) == tuple:
                            ...
                            # print(path_to_squares + file_name, 'AXAXAXpath_to_squares')
                            ...
                            # print(path_to_squares, 'AXAXAXAXAXAX')
                            ...
                            uploaded_answer_1(path_to_squares + file_name, path_to_squares + '/' + 'current_answers',
                                              path_to_squares + '/' + 'answers')
                            print(path_to_squares, 'path_to_squarespath_to_squarespath_to_squarespath_to_squarespath_to_squarespath_to_squares')
                            # print(first, second)
                            first = []
                            for i in one:
                                try:
                            ...
                            if len(first) != len(second):
                                ...
                                bot.send_message(message.chat.id, 'алгоритм сообщает, что эти файлы не из одного набора тестов'
                                                                  '.')
                            else:
